...
public class LeadCrateContainer extends Container {

    public TileLeadCrate tileEntity;

    public LeadCrateContainer(ContainerType<?> type, int id) {
        super(type, id);
    }

    // Client
    public LeadCrateContainer(int id, PlayerInventory inv, PacketBuffer buf) {
        super(LEAD_CRATE_CONTAINER.get(), id);
        this.init(inv, this.tileEntity = (TileLeadCrate) inv.player.level.getBlockEntity(buf.readBlockPos()));
    }

    // Server
    public LeadCrateContainer(int id, PlayerInventory inv, TileLeadCrate tile) {
        super(LEAD_CRATE_CONTAINER.get(), id);
        this.init(inv, this.tileEntity = tile);
    }

    public void init(PlayerInventory playerInv, TileLeadCrate tile) {
